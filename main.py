...
def get_marks_list():
    try:
        ...
        query = """
            ...
            ORDER BY s.studname
        """
        marks = execute_query(query, (term, session['teachID']), fetch=True)
...
def get_detailed_marks(stud_id):
    try:
        term = request.args.get('term', 'Term 1')
        query = "SELECT remark FROM marks WHERE studID = %s AND term = %s"
        marks_data = execute_query(query, (stud_id, term), fetch=True)
        
        if marks_data:
            # Return only the remark since detailed marks are handled in frontend
            return jsonify({
                'success': True, 
                'marks_details': None,  # Frontend will handle detailed marks
                'remark': marks_data[0]['remark']
            })
        else:
            # No marks found for this term
            return jsonify({'success': True, 'marks_details': None, 'remark': ''})
    except mysql.connector.Error as e:
        return jsonify({'success': False, 'message': str(e)})
...
def save_detailed_marks():
    try:
        data = request.json
        student_info = execute_query("SELECT studname FROM students WHERE studID = %s AND teachID = %s", 
                                   (data['studID'], session['teachID']), fetch=True)
        if not student_info:
        ...
        existing_mark = execute_query("SELECT markID FROM marks WHERE studID = %s AND term = %s", 
                                    (data['studID'], term), fetch=True)
                
        if existing_mark:
            query = "UPDATE marks SET marks=%s, studname=%s, ovrscore=%s, meanscore=%s, remark=%s WHERE studID=%s AND term=%s"
            params = (marks_value, studname, ovrscore, meanscore, remark, data['studID'], term)
        else:
            query = "INSERT INTO marks (marks, studID, studname, ovrscore, meanscore, remark, term) VALUES (%s, %s, %s, %s, %s, %s, %s)"
            params = (marks_value, data['studID'], studname, ovrscore, meanscore, remark, term)
        
        execute_query(query, params)
        return jsonify({'success': True, 'message': f'Marks saved successfully for {term}!'})
    except mysql.connector.Error as e:
...
def generate_detailed_report():
    try:
        ...
        # Get the student's marks and info for the specific term
        student_marks_query = """
        ...
        """
        student_data = execute_query(student_marks_query, (term, stud_id, session['teachID']), fetch=True)
        ...
        if all_subjects:
            ...
            for subject in all_subjects:
                subject_key = subject['subname'].lower().replace(' ', '_').replace('and', '').replace(' ', '_')
                subjects_for_report[subject_key] = {
                    ...
                    # Use the overall mean score and grade for every subject
